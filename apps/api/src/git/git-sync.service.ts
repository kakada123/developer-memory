...
export class GitSyncService implements OnModuleInit {
  ...
  private async run(project:Project):Promise<void>{const started=Date.now();try{
    await this.commands.validateRepository(project.path);const [head,branch,newest,existingCommits]=await Promise.all([this.commands.head(project.path),this.commands.branch(project.path),this.repository.newest(project.id),this.repository.count(project.id)]);
    let rebuilt=false; if(newest&&!await this.commands.isAncestor(project.path,newest.hash,head))rebuilt=true;
    this.progress.update(project.id,{status:'READING_COMMITS',percentage:15,message:'Reading commit history…'});
    const output=await this.commands.log(project.path,historyLimit,rebuilt?undefined:newest?.hash);const commits=this.parser.parseLog(output);
    ...
    const changedFiles=await this.repository.replaceOrAdd(project.id,commits,rebuilt);const count=await this.repository.count(project.id);const now=new Date();
